...
| F1 | 수입 가능 판정 | 병찬 | 원재료 허용/금지 판정, 기준치 비교, 법령 기반 AI 분석 |
...
| F5 | 한글 표시사항 시안 | 세연 | 2단계 교차검증 + Claude Sonnet 4 SSE 스트리밍 시안 생성 |
...
- **AI/LLM**: Anthropic Claude (f0, F1, F5), OpenAI GPT-4o (F2, F4)
- **벡터 DB**: Pinecone (F2: samc-a, F3: samc-law-f3, F4: samc-feature4-laws, F5: f5-law-chunks)
- **임베딩**: sentence-transformers multilingual-e5-large (F4), Voyage-3 (F5)
- **DB**: Supabase (PostgreSQL) + asyncpg 직접 연결 (F1)
...
│   │   ├── connection.py          # F1 asyncpg 커넥션 풀
│   │   ├── migrations/            # F1 SQL 마이그레이션 (001~009)
│   │   ├── seed/                  # F1 초기 데이터 (01~05)
...
| `f1_` | F1 수입판정 | `f1_allowed_ingredients`, `f1_forbidden_ingredients`, `f1_additive_limits`, `f1_safety_standards`, `f1_ingredient_synonyms` |
