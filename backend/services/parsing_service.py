...
5. **제조공정 코드 변환이 핵심입니다.** 제조공정도(process) 텍스트에서 공정 설명을 하나하나 분석하여, 아래 식약처 공식 공정 코드 참조 테이블에서 가장 가까운 코드를 매핑하세요.
   ...
   - **공정 순서 결정 규칙 (매우 중요):**
     1. 텍스트에 "1. XXX, 2. YYY" 처럼 명시적 번호가 있으면 그 번호 순서대로 step_number를 부여합니다.
     2. 번호가 없는 플로우차트라면, **제공된 텍스트에 나열된 순서 그대로** step_number 1, 2, 3... 을 순서대로 부여합니다. 텍스트는 이미 위→아래 시각적 순서로 정렬되어 있으므로, 임의로 순서를 바꾸지 마세요.
